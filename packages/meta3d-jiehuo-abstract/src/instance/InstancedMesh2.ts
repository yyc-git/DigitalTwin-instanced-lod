import { BufferGeometry, Camera, Color, ColorRepresentation, DynamicDrawUsage, Frustum, InstancedBufferAttribute, InstancedMesh, Intersection, Material, Matrix4, Mesh, Object3D, Raycaster, Sphere, Vector3 } from 'three';
import { InstancedEntity } from './InstancedEntity';
// import { InstancedMeshBVH } from './InstancedMeshBVH';
import { state } from '../type/StateType';
import { findAllSources, getWorldBoundingBox, getIndex, isVisible } from './Instance';
...
export class InstancedMesh2<T = {}, G extends BufferGeometry = BufferGeometry, M extends Material = Material> extends InstancedMesh<G, M> {
  ...
  public config: InstancedMesh2Params<T>
  private _show = []
  private _hide = []
  ...
  constructor(geometry: G, material: M, count: number, config: InstancedMesh2Params<T>) {
    ...
    const color = config.color !== undefined ? new Color().set(config.color) : undefined;
  ...
  private setInstancesVisibility() {
    ...
    // let startIndex = 0;
    // let endIndex = this._hide.length - 1

    // while (endIndex >= startIndex) {
    //   this.swapInstances(hide[startIndex], this.count - 1)
    //   startIndex += 1
    //   this.count -= 1
    // }
    ...
    if ((this._show.length + this._hide.length) != this.instances.length) {
    ...
    this._hide.forEach(entity => {
      if (entity._internalId >= self.count) {
        return
      }

      self.swapInstances(entity, self.count - 1)
    ...
    this._show.forEach(entity => {
      if (entity._internalId <= self.count - 1) {
        return
      }

      self.swapInstances(entity, self.count)
  ...
  private swapInstances(instanceFrom: Entity<T>, idTo: number) {
  ...
  public handleBeforeUpdateCulling() {
    this._show.length = 0;
    this._hide.length = 0;
  }

  // public getShowAndHideForCulling() {
  //   return [_show, _hide]
  // }


  public updateCulling(state: state, camera: Camera) {
    if (!this.getPerObjectFrustumCulled()) {
      return
    }

    this.handleBeforeUpdateCulling()
    ...
    findAllSources<Object3D>(state, this.id).forEach(source => {
      worldPosition = source.getWorldPosition(worldPosition)

      let index = getIndex(state, source)

      if (!isVisible(state, source)) {
        this.hide(index)
        return
      }

      if (frustum.intersectsBox(getWorldBoundingBox(state, source))) {
        this.show(index)
      }
      else {
        this.hide(index)
  ...
  public updateVisible() {
    ...
    if (this._show.length > 0 || this._hide.length > 0) {
      // if (this._hide.length > 0) {
      let needUpdate = this.setInstancesVisibility();
  ...
  public show(index) {
    this._show.push(this.instances[index])
  }

  public hide(index) {
    this._hide.push(this.instances[index])
  }
  ...
  //     // TODO find source lod when Instance->setVisible, then get its visible and boundingSphere/Box
  ...
  public getPerObjectFrustumCulled() {
    return this._perObjectFrustumCulled
  }

  public setMatrixWorld(index, matrixWorld) {
    this.instances[index].setMatrixWorld(matrixWorld)
    this.markNeedsUpdate()
  }

  public setColor(index, color) {
    this.instances[index].setColor(color)
    this.markNeedsUpdate()
  }
